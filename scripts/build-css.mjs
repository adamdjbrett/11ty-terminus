...
const sassRoot = path.join(cwd, "sass");
...
if (!fs.existsSync(sassRoot)) {
  console.log("[build:css] No sass directory; skipping.");
  process.exit(0);
}

const files = walk(sassRoot);
if (!files.length) {
...
for (const src of files) {
  ...
  try {
    const result = compile(src, {
      loadPaths: [sassRoot, cwd, path.join(cwd, "node_modules")],
      style: "expanded",
      sourceMap: false,
    });
